...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!existUser) {
      return next(
        new CustomError("User does not exist , Register before login!", 400)
      );
    }
...
exports.passwordReset = async (req, res, next) => {
  try {
    ...
    if (!foundUser) {
      return next(
        new CustomError("Forgot password link is expired or invalid!", 404)
      );
    }
    ...
    if (password !== confirmPassword) {
      return next(
        new CustomError("Password and Confirm Password doesn's matched!", 404)
      );
    }
